  ...
  try {
    ...
    return {
      ...
      entities: (data.entities || []).map((e: any) => ({
        ...
        lineEnd: e.line_end,
        introducingCommit: e.introducing_commit,
        introducingDate: e.introducing_date,
        lastModifiedCommit: e.last_modified_commit,
        lastModifiedDate: e.last_modified_date,
        changeCount: e.change_count,
        activeAuthors: e.active_authors,
        relatedAdrs: e.related_adrs,
      })),
      relationships: (data.relationships || []).map((r: any) => ({
        sourceName: r.source_name,
        sourcePath: r.source_path,
      ...
      historicalChanges: (data.historical_changes || []).map((c: any) => ({
        ...
        isIntroducing: c.is_introducing,
        prNumber: c.pr_number,
      })),
      relatedPrs: (data.related_prs || []).map((p: any) => ({
        ...
        url: p.url,
        linkedIssueNumbers: p.linked_issue_numbers,
      })),
      relatedIssues: (data.related_issues || []).map((i: any) => ({
      ...
      designConstraints: (data.design_constraints || []).map((dc: any) => ({
        id: dc.id,
        domain: dc.domain,
        constraintText: dc.constraint_text,
        sourceDocPath: dc.source_doc_path,
